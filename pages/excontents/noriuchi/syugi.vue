<template>
    <div class="report_content">
        <div class="report_header">
            <h2 class="report_title">乗り打ち計算機（祝儀型）</h2>
            <p class="update_time">トラブルを回避するために考案した計算方法です。オススメの計算方法です。</p>
            <p class="bread">
                <NuxtLink to="/">Shimesaba</NuxtLink> ＞
                <NuxtLink to="/excontents/">コンテンツ</NuxtLink> ＞
                <NuxtLink to="/excontents/noriuchi">乗り打ち計算機</NuxtLink> ＞
                <NuxtLink to="/excontents/noriuchi/syugi">祝儀型</NuxtLink>
            </p>
            ...
            <div class="share_area">
                ...
                <div class="fb-like" data-href="https://shimesaba.jp/excontents/noriuchi/syugi" data-layout="button_count" data-action="like" data-show-faces="false" data-share="true" style="width: 155px;"></div>
            </div>

        </div>

        <div class="report">
            <p>個人収支がプラスとなったメンバーのプラス収支分を分配するタイプの乗り打ちです<br>
            <br>
            共有財布型だと投資に対するそれぞれの責任が小さくなるため過剰な投資をする可能性があります<br>
            このタイプの場合、投資は全て自分の責任となるので投資を抑制する事が出来ます<br>
            <br>
            短所は投資をしなければ負ける事は無い点です。</p>
            <p>計算方法：個人収支がプラスのメンバーのプラス分を合算し、人数で割って分配します<br>
            メンバーそれぞれの投資額、回収額を入力してください<br>
            （使わない欄は0にして置いてください）</p>
            <fieldset>
                <legend>メンバー1</legend>
                <form id="member1">
            ...
            <fieldset>
                ...
                <form id="ans">
                    <p>
                        ...
                        <label>pool：</label>
                        <input type="number" name="pool" disabled="" :value="total_pool">円<br>
                        <label>祝儀：</label>
                        <input type="number" name="syugi" disabled="" :value="syugi">円
                    </p>
                    <p>
                        <label>メンバー1：</label>
...
export default {
        ...
        changeValue() {
            for(let i = 1; i <= 5; i++) {
                if(this["m"+i+"o"] == 0 && this["m"+i+"i"] == 0){
                    ...
                    if(kojin > this.syugi){
                        this["m"+i+"r"] = (kojin - this.syugi) + "円　他のメンバーへ渡す(個人成績："+kojin+")";
                        this["m"+i+"f"] = this.syugi;
                    } else if(kojin > 0){
                        this["m"+i+"r"] = (this.syugi - kojin) + "円　他のメンバーから受け取る(個人成績："+kojin+")";
                        this["m"+i+"f"] = this.syugi;
                    }else{
                        this["m"+i+"r"] = (this.syugi) + "円　他のメンバーから受け取る(個人成績："+kojin+")";
                        this["m"+i+"f"] = kojin + this.syugi;
                    }
                }
            }
        },
    ...
    computed: {
        ...
        total_pool() {
            ...
            if(!(this.m1i == 0 && this.m1o == 0) && (this.m1o - this.m1i) > 0) {
                pool += this.m1o - this.m1i;
            }
            if(!(this.m2i == 0 && this.m2o == 0) && (this.m2o - this.m2i) > 0) {
                pool += this.m2o - this.m2i;
            }
            if(!(this.m3i == 0 && this.m3o == 0) && (this.m3o - this.m3i) > 0) {
                pool += this.m3o - this.m3i;
            }
            if(!(this.m4i == 0 && this.m4o == 0) && (this.m4o - this.m4i) > 0) {
                pool += this.m4o - this.m4i;
            }
            if(!(this.m5i == 0 && this.m5o == 0) && (this.m5o - this.m5i) > 0) {
                pool += this.m5o - this.m5i;
            }

            return pool;
        },
        syugi() {
            return this.member_cnt > 0 ? this.total_pool / this.member_cnt : 0;
        }
    },
    mounted() {
